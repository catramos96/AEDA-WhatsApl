...
void Comunidade::updateUtilizadoresInativos() {
	vector<Utilizador*>::iterator c;

	for (c = comunidade.begin(); c != comunidade.end(); c++) {
		if ((*c)->inativo() > 30) {
			utilizadoresInativos.insert(*c);
			c = comunidade.erase(c);
			c--;
		}
	}
	...
	while (it != utilizadoresInativos.end()) {
		if((*it)->inativo() < 30 ){
			comunidade.push_back(*it); //coloca o utilizador na comunidade
			it = utilizadoresInativos.erase(it);
		}
		else
			it++;
	}
