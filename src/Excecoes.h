...
public:
	 ...
	 */
	UtilizadorInexistente(string u) {
		this->u = u;
	}
	;
	/**
...
public:
	 ...
	 */
	ConversaJaExiste(vector<string> p) {
		participantes = p;
	}
	;
	/**
...
public:
	/**
	 * @brief Construtor.
	 * Inicializa o membro privado login com o login recebido
	 * @param login Login do utilizador que criou a excecao.
	 */
	MembroInexistente(string login) {
		this->login = login;
	}
	;
	/**
	 * @brief Funcao que devolve o utilizador.
	 * @return Login do utilizador que criou a excecao
	 */
	string getLogin() const {
		return login;
...
public:
	 ...
	 */
	DataInvalida(int d, int m, int a) {
		dia = d;
		mes = m;
		ano = a;
	}
